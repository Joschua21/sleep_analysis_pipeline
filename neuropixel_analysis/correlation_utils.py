...
def cross_correlation_batch_fft(spike_data, lag_range_ms=200, 
                               lag_resolution_ms=5, min_firing_rate=0.001,
                               batch_size=50):
    """
    Process multiple pairs simultaneously using vectorized operations
    """
    from scipy.signal import correlate
    # Setup (same as before)
    firing_rates = np.mean(spike_data, axis=1)
    active_indices = np.where(firing_rates > min_firing_rate)[0]
    n_active = len(active_indices)
    
    active_data = spike_data[active_indices]
    active_data_norm = (active_data - np.mean(active_data, axis=1, keepdims=True)) / \
                       np.std(active_data, axis=1, keepdims=True)
    
    max_lag_bins = int(lag_range_ms / lag_resolution_ms)
    lag_bins = np.arange(-max_lag_bins, max_lag_bins + 1) * int(lag_resolution_ms)
    
    # Vectorized approach: correlate each neuron with ALL others at once
    n_pairs = n_active * (n_active - 1) // 2
    peak_correlations = np.zeros(n_pairs)
    peak_lags = np.zeros(n_pairs)
    
    pair_idx = 0
    with tqdm(total=n_active, desc="Batch FFT") as pbar:
        for i in range(n_active):
            # Correlate neuron i with all subsequent neurons at once
            remaining_neurons = active_data_norm[i+1:]
            if len(remaining_neurons) == 0:
                break
                
            # Batch correlate: one vs many
            xcorr_batch = np.array([
                correlate(active_data_norm[i], neuron_j, mode='full')
                for neuron_j in remaining_neurons
            ]) / active_data.shape[1]  # Normalize
            
            # Extract peaks for this batch
            center = xcorr_batch.shape[1] // 2
            lag_indices = center + lag_bins // lag_resolution_ms
            
            for j, xcorr_full in enumerate(xcorr_batch):
                # Extract valid lags
                valid_mask = (lag_indices >= 0) & (lag_indices < len(xcorr_full))
                if np.any(valid_mask):
                    lag_correlations = xcorr_full[lag_indices[valid_mask]]
                    peak_idx = np.argmax(np.abs(lag_correlations))
                    peak_correlations[pair_idx] = lag_correlations[peak_idx]
                    peak_lags[pair_idx] = lag_bins[valid_mask][peak_idx]
                
                pair_idx += 1
            
            pbar.update(1)
    
    return {'peak_correlations': peak_correlations, 'peak_lags': peak_lags, 'n_pairs': n_pairs}
